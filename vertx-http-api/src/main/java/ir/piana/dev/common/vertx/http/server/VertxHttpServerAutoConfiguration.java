...
import ir.piana.dev.common.vertx.http.websocket.WebSocketEstablisherRequestHandler;
...
//        vertxRouter.route("/ws-test")
//                .handler(routingContext -> {
//                    // Ensure the request has not ended rc.request().isEnded() == false
//                    assert !routingContext.request().isEnded();
//                    // Pause the request (It is used by the JSSock implementation) - JSSock implementation tests if it has ended but in this case it always fails
...
//                });
        ...
        for (HttpRouteItem httpRouteItem : httpRouteItems) {
            ...
            if (httpRouteItem.getResponse() != null) {
                if (httpMethod == null) {
                    route = vertxRouter.route(httpRouteItem.getPath().startsWith("/") ?
                            ...
                            .handler(SessionHandler.create(LocalSessionStore.create(vertx)).setNagHttps(false))
                            .handler(routingContext -> {
                                generateResponse(httpRouteItem, routingContext.response(),
                                        handlerResponseBuilder.withoutBody().build(), authPhraseConsumable);
            ...
            route.failureHandler(routingContext -> {
                ...
                Class handlerClass = vertxHandlerClassMap.get(httpRouteItem.getHandlerClass());
                ...
                if (handlerClass.isAnnotationPresent(ResponseAsView.class)) {
                    ...
                    routingContext.response().setStatusCode(httpStatus.getStatusCode())
                            .putHeader("Content-Type", "application/json; charset=utf-8")
                            .end(JsonTargetBuilder.asObject()
                                    .add("message", messageSource.getMessage(
                                            throwable.getMessage(),
                                            new Object[0],
                                            throwable.getMessage(),
                                            Locale.forLanguageTag("fa"))).build().getJsonObject().toBuffer());
                }
            });
        }
    ...
    private void handle(
        ...
        try {
            ...
            if (WebSocketEstablisherRequestHandler.class.isAssignableFrom(handlerClass)) {
                assert !routingContext.request().isEnded();
                /**
                 * Pause the request (It is used by the JSSock implementation) -
                 * JSSock implementation tests if it has ended but in this case it always fails
                 * */
                routingContext.request().pause();
                Thread.ofVirtual()
                        .start(() -> {
                            ...
                            requestHandler.establish(routingContext, handlerRequest, ((HandlerResponse) result).getAuthPhrase());
                        });
            } else {
                deferredResult.setResultHandler(
                        handlerResponse -> ok(applicationContext, handlerClass, routingContext, handlerResponse,
                                authPhraseConsumable,
                                templateEngine,
                                httpRouteItem));

                /*final boolean isResponseAsView = handlerClass.isAnnotationPresent(ResponseAsView.class);*/
    ...
    private void ok(ApplicationContext applicationContext, Class handlerClass, RoutingContext routingContext,
        ...
        if (handlerResponse instanceof HandlerResponse<?>) {
            generateResponse(
                    routeItem, routingContext.response(),
            ...
            templateEngine.getTemplateEngine().render(jsonObject,
                    ...
                    .toCompletionStage().whenComplete((buffer, throwable) -> {
                        if (((HandlerModelAndViewResponse) handlerResponse).getAuthPhrase() != null && authPhraseConsumable != null)
    ...
    private void generateResponse(
            ...
            HttpServerResponse response,
            HandlerResponse handlerResponse,
            AuthPhraseConsumable authPhraseConsumable) {
        ...
        if (handlerResponse.getAuthPhrase() != null && authPhraseConsumable != null)
            authPhraseConsumable.produce(response, handlerResponse.getAuthPhrase());
